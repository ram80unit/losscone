...
for i = 1:length(lats),
    ...
    for j = 1:length(lons),
        ...
        Bs = igrf(gh,lats(i),lons(j),sat_alt);
...
locationerror = 10:10:150;     % error in km
...
for i = 1:length(lats),
    ...
    for j = 1:length(lons),
        ...
        [newlat,newlon] = reckon(lats(i),lons(j),locationerror(m)/Re*180/pi,direction);
        
        Bs = igrf(gh,newlat,newlon,sat_alt);
...
maxdirErr(m) = max(max(abs(dirErr)));

%lca2 = real(asind(sqrt(Bsat2./Bmir2)));

%maxlcaErr(m) = max(max(abs(lca-lca2)));
